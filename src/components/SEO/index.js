...
const SE0 = ({ title, meta_title, meta_desc, featuredimage, slug, date }) => {
  const postURL = config.siteUrl + slug;
  const realPrefix = config.pathPrefix === "/" ? "" : config.pathPrefix;
  const image = config.siteUrl + realPrefix + featuredimage;
    ...
    itemListElement: [
      {
        ...
        item: {
          "@id": config.siteUrl,
          name: "Home",
          image: config.siteUrl + "/img/logo.png",
        },
      },
      {
        "@type": "ListItem",
  ...
  const blogPostingSchemaOrgJSONLD = {
    ...
    publisher: {
      ...
      logo: {
        "@type": "ImageObject",
        url: config.siteUrl + "/img/logo.png",
      },
    },
